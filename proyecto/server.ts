...
import db from './database/db.ts'; // 👈 Nueva importación
...
app.post('/api/habits', (req, res) => {
  ...
  db.run('INSERT INTO habits (id, name, color) VALUES (?, ?, ?)', id, name, color || '#60a5fa', function(err) {
    ...
    db.get('SELECT MAX(position) AS maxPos FROM habit_order', (err, row: any) => {
...
app.patch('/api/habits/:id/move', (req, res) => {
  ...
  db.get('SELECT position FROM habit_order WHERE habit_id = ?', id, (err, row: any) => {
    if (err || !row) return res.status(404).json({ error: 'Habit not found' });
    const pos = row.position;
    const newPos = pos + direction;
    db.get('SELECT habit_id FROM habit_order WHERE position = ?', newPos, (err, swap: any) => {
      if (swap) {
        ...
        db.run('UPDATE habit_order SET position = ? WHERE habit_id = ?', pos, swap.habit_id, (err) => {
          if (err) return res.status(500).json({ error: 'Failed to move habit' });
...
app.get('/api/logs/:date', (req, res) => {
  ...
  db.all('SELECT habit_id FROM habit_logs WHERE date = ?', date, (err, rows: any[]) => {
...
app.delete('/api/logs/:date', (req, res) => {
  ...
  db.run('DELETE FROM habit_logs WHERE date = ?', date, function(err) {
    if (err) return res.status(500).json({ error: 'Failed to clear day' });
    res.json({ message: 'Day cleared' });
  });
});

app.get('/api/stats', (req, res) => {
  db.get('SELECT COUNT(*) AS count FROM habits', (err, total: any) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch stats' });
    const today = new Date().toISOString().slice(0, 10);
    db.get('SELECT COUNT(DISTINCT habit_id) AS count FROM habit_logs WHERE date = ?', today, (err, completed: any) => {
      if (err) return res.status(500).json({ error: 'Failed to fetch stats' });
      res.json({
